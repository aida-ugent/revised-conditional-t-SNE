...
#   from fast_ctsne import fast_ctsne
#   import numpy as np
#   X = np.random.randn(1000, 50)
#   labels = np.random.randint(2, size=1000)
#   Z = fast_ctsne(X, labels)
#
...
def fast_ctsne(
    ...
    labels: 1D numpy array
        Array of labels (n) to be factore out from the embedding.
        Labels should be integers starting from 0.
    alpha: float between 0 and 1
        Factor to decrease affinity between same-labeld points.
    perplexity: double
    ...
    K: int
        ...
        is -1.
    keep_nonzero: int
        When using a nonzero perplexity, the high-dimensional similarities are
        approximated by only keep_nonzero * perplexity neighbors. Default 3.
        Set to -1 when perplexity is not used.
    nterms: int
    ...
    alpha_list: list
        A list of alphas with values to be used for the different labels,
        e.g. alpha_list[0] will be used for all points with label 0.
    ...
    print_output: boolean
        If True prints the output. Default is False.
    hd_tdist: boolean
        If True uses a t-distribution in the HD space. Default is False.
    hd_df: double
        Controls the degree of freedom of the HD t-distribution. Default is 1.0.
    fixed_perp: bool
        If True, the variance for the Gaussian kernel in HD space will be fixed
        before multiplying with the alpha/beta terms. Default is True (var on p_i)
    ...
    label_counts = [sum(labels == val) for val in unique_labels]
    assert (
        np.max(labels) < num_label_vals
    ), f"Labels must be integers starting between 0 and {num_label_vals - 1}."
    assert np.min(labels) >= 0, "Labels must be integers starting from 0."

    # check alpha and compute beta
    if alpha_list is not None:
        assert (
            len(alpha_list) == num_label_vals
        ), f"alpha_list must have the same length as the number of unique labels {num_label_vals}"
        # compute betas
        beta_list = list()
        for a in alpha_list:
            beta_list.append(get_beta(label_counts=label_counts, n=n, alpha=a))
        alpha = 0
        beta = 0
        ...
        ), "alpha must be in (0,1]. With alpha = 1 we compute normal t-SNE."

        beta = get_beta(label_counts=label_counts, n=n, alpha=alpha)
        print("alpha = {:.6f}, beta = {:.6f}".format(alpha, beta))

    # normal t-SNE configuration
    ...
    # cannot find more neighbors than datapoints
    if keep_nonzero * perplexity > X.shape[0]:
        raise ValueError(
            "keep_nonzero is too large. In combination with perplexity"
            + f"{perplexity} we'd need a dataset with {keep_nonzero * perplexity} points."
        )
    ...
    if search_k is None:
        if perplexity > 0:
            search_k = keep_nonzero * perplexity * n_trees
        elif perplexity == 0:
            search_k = keep_nonzero * np.max(perplexity_list) * n_trees
        else:
            search_k = K * n_trees
    ...
    with open(os.getcwd() + "/" + infile, "wb") as f:
        ...
        f.write(struct.pack("=i", num_label_vals))  # ct-SNE
        f.write(struct.pack("=d", alpha))  # ct-SNE
        f.write(struct.pack("=d", beta))  # ct-SNE
        if alpha == 0:
            for a in alpha_list:
                f.write(struct.pack("=d", a))
            for b in beta_list:
                f.write(struct.pack("=d", b))
        f.write(labels.tobytes())  # ct-SNE
        ...
        f.write(struct.pack("=i", int(keep_nonzero)))
        ...
        f.write(struct.pack("=?", hd_tdist))
        f.write(struct.pack("=d", hd_df))
        f.write(struct.pack("=?", fixed_perp))
    ...
    try:
        result = subprocess.check_output(
            [
                os.path.dirname(os.path.realpath(__file__)) + "/bin/fast_ctsne",
                version_number,
                infile,
                outfile,
                "{}".format(nthreads),
        ...
        if os.path.isfile(os.path.join(os.getcwd(), infile)):
            os.remove(os.path.join(os.getcwd(), infile))
        if os.path.isfile(os.path.join(os.getcwd(), outfile)):
            os.remove(os.path.join(os.getcwd(), outfile))
        print(
            f"t-SNE did not execute correctly. Error {grepexc.returncode}, Result {grepexc.output.decode()}"
        )


def get_beta(label_counts, n, alpha):
    ratio = sum(nl * (nl - 1) / (n * (n - 1)) for nl in label_counts)
